...
public class ExtentManager {
    ...
    public static ExtentHtmlReporter createHtmlReporter(String filePath){
        ...
        htmlReporter.config().setReportName("自动化项目测试报告");// 报告的名称
        return htmlReporter;
    }

    public static ExtentXReporter createExtentXReporter() {
        ExtentXReporter extentx = new ExtentXReporter("127.0.0.1",27017);// 关联MongoDB的路径
        extentx.config().setProjectName("Maven_Test2");// 项目的名称
        extentx.config().setReportName("Report_191012");// 单次report的名称
