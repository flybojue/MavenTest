...
public class SendMail {
    public static boolean sendHtmlMail(MailInfo mailInfo) {
        ...
        try {
            ...
            //设置抄送者info
            String[] cclist = mailInfo.getCcAddress();//创建邮件的抄送者地址
            InternetAddress[] cc = new InternetAddress[cclist.length];
            for (int i = 0; i < cclist.length; i++) {
                cc[i] = new InternetAddress(cclist[i]);
            }
            mailMessage.setRecipients(Message.RecipientType.CC, cc);//设置邮件消息的抄送者
            ...
            //若有附件，将附件的地址写进attachfiles.properties配置文件内
